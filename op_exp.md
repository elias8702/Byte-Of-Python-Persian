...
بیشتر فراخوانی‌ها(خطوط منطقی) که می‌نویسید، دارای _expressions(عبارت‌ها)_ هستند. برای مثال  `2 + 3`. عبارات‌ها را می‌توان به دو بخش تقسیم کرد عملوندها و عبارت‌ها.
...
عملوند‌های که تا به حال ندیده‌ایم در فصول بعدی توضیح داده خواهد شد.

عملوند‌های که  _same precedence(اولویت یکسان)_ دارند همان طور که در جدول بالا می‌بیند. برای مثال `+` و `-` که اولویت یکسانی دارند. تمامی انان در یک خط نوشته شده‌اند.
...
طول و عرض مستطیل با همان اسم در متغییر ذخیره می‌شوند. ما برای محاسبه طول و عرض مستطیل از عبارت‌ها استفاده می‌شود. ما نتیجه را در عبارت `length * breadth (طول و عرض)` در +area(مساحت)+ ذخیره می‌کنیم. و بعد با استفاده از تابع +print+ خروجی `2 * (length + breadth)` را چاپ می‌کنیم. همین طور که مشاهده می‌کنید دراینجا ما از محتویات عبارت‌ها استفاده می‌کنیم.

همچنین این نکته را در نظر بگیرید که پایتون به چه زیبایی خروجی را چاپ می‌کند. حتی با اینکه ما بین `'Area is'` و متغییر `area` فاصله نگذاشتیم. پایتون برای ما این فاصله‌های را گذشته است. برای این است که ما یک خروجی تمیز و چاپ قابل خواندن داشته باشیم(این به ان دلیل است که دیگر نیاز نیست برای فاصله‌ی خطوط نگران باشیم).این یکی از مثال‌ها‌ی است که پایتون زندگی برنامه‌نویسان را اسوده‌تر می‌کند.

## خلاصه

چگونگی استفاده از عملوند‌ها و عبارت‌ها را دیدیم. این موضوع یک ساختار مهم در برنامه‌نویسی است. در فصول بعدی خواهیم دید چگونه دربرنامه‌هایمان استفاده کنیم و همچنین فراخوانی‌ها.
